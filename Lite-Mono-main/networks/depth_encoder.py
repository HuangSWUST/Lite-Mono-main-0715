...
class XCA(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads)  # 在通道数维度进行拆分，得到检测头
        qkv = qkv.permute(2, 0, 3, 1, 4)  # (3,B,8,N,C/8)
        q, k, v = qkv[0], qkv[1], qkv[2]  # (B,8,N,C/8)

        q = q.transpose(-2, -1)  # 最后两个维度做转置
        k = k.transpose(-2, -1)
        v = v.transpose(-2, -1)  # (B,8,C/8,N)

        q = torch.nn.functional.normalize(q, dim=-1)  # 归一化
        k = torch.nn.functional.normalize(k, dim=-1)

        attn = (q @ k.transpose(-2, -1)) * self.temperature  # QTxK [B,8,C/8,C/8]
        attn = attn.softmax(dim=-1)  # 建立指数映射，再归一化
        attn = self.attn_drop(attn)

        x = (attn @ v).permute(0, 3, 1, 2).reshape(B, N, C)  # 叉乘v=[B,8,C/8,N]->[B,N,8*C/8]
        x = self.proj(x)  # 线性映射
        x = self.proj_drop(x)
        return x
...
class LGFI(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, expan_ratio=6,
        ...
        self.norm_xca = LayerNorm(self.dim, eps=1e-6)  # 层归一化，针对样本的不同特性
    ...
    def forward(self, x):
        ...
        # XCA
        B, C, H, W = x.shape
        x = x.reshape(B, C, H * W).permute(0, 2, 1)  # 调整通道

        if self.pos_embd:  # [True, False, False]位置编码
            pos_encoding = self.pos_embd(B, H, W).reshape(B, -1, x.shape[1]).permute(0, 2,
                                                                                     1)  # (B,H,W)位置编码,reshape(B,-1,H*w),通道调整后(B,H*W,-1)
            x = x + pos_encoding

        x = x + self.gamma_xca * self.xca(self.norm_xca(x))  # x+gama*(B,N,C)

        x = x.reshape(B, H, W, C)

        # Inverted Bottleneck
        x = self.norm(x)  # 层归一化
        x = self.pwconv1(x)  # PW卷积(C-6C)
        x = self.act(x)  # GELU激活
        x = self.pwconv2(x)  # PW卷积(6C-C)
        if self.gamma is not None:
            x = self.gamma * x
        x = x.permute(0, 3, 1, 2)  # (N, H, W, C) -> (N, C, H, W)
...
class LiteMono(nn.Module):
    ...
    def __init__(self, in_chans=1, model='lite-mono', height=192, width=640,
        ...
        stem1 = nn.Sequential(
            Conv(in_chans, self.dims[0], kSize=3, stride=2, padding=1, bn_act=True),  # 3x3,48,stride=2
            Conv(self.dims[0], self.dims[0], kSize=3, stride=1, padding=1, bn_act=True),  # [3x3,48]x2
        ...
        self.stem2 = nn.Sequential(
            ...
            # +1concat,3x3,48,stride=2
        )

        self.downsample_layers.append(stem1)  # downsample_layers[0]
        ...
        dp_rates = [x.item() for x in torch.linspace(0, drop_path_rate, sum(self.depth))]  # [0:0.2],s=18
        cur = 0  # 模块计数器
        for i in range(3):
            stage_blocks = []
            for j in range(self.depth[i]):  # [4,4,10]
                if j > self.depth[i] - global_block[i] - 1:  # global_block[1,1,1] true j=3
                    if global_block_type[i] == 'LGFI':
                        ...
                        raise NotImplementedError
                else:  # j=0,1,2 stage2:3组,stage3:3组,stage4:9组
                    stage_blocks.append(
    ...
    def forward_features(self, x):
        features = []
        x = (x - 0.39) / 0.215  # 输入图像归一化

        x_down = []
        for i in range(4):
            x_down.append(self.input_downsample[i](x))  # 每层的降采样AvgPool结果

        tmp_x = []  # 暂存数据用于concat,[stage2.downsample,stage2.output,avg2]
        x = self.downsample_layers[0](x)  # convStem1
        x = self.stem2(torch.cat((x, x_down[0]), dim=1))  # concat+stage2.downsample
        tmp_x.append(x)

        for s in range(len(self.stages[0]) - 1):
            x = self.stages[0][s](x)  # stage2 output
        x = self.stages[0][-1](x)
        tmp_x.append(x)
        features.append(x)  # 特征图
        # 执行stage3,stage4
        for i in range(1, 3):
